	...
	$.Elastislide.prototype = {
		...
		_initEvents : function() {
            ...
            if(this.options.autoplay == true){
				...
				var totalSpace = this._calculateItemsTotalSpace();
				...
				window.setInterval(function(){
					...
					if(totalSpace > translation + visibleSpace)
					{
						...
						for(var i=0;i<self.$items.length;i++){
						   if(i==itemNo){
						     translation += $(self.$items[i]).outerWidth();
							 itemNo =  itemNo+1;
							 self.itemNo = itemNo;
							 break;
						   }
...
//						translation += visibleSpace/4;
        ...
        _calculateItemsTotalSpace:function(){
		    var that =this;
			var totalSpace=0;
				
			$.each(that.$items, function (index,item) {
				totalSpace = totalSpace+$(item).outerWidth();
			});
			return totalSpace;
		},
		_slide : function( dir, tvalue ) {
			...
			var self = this,
				...
				totalSpace = this._calculateItemsTotalSpace(),
				// visible width/height
			...
			if( tvalue === undefined ) {
				...
				var amount = (this.fitCount * itemSpace)/4;
					for(var i=0;i<self.$items.length;i++){
					   if(i==self.itemNo){
						 amount = $(self.$items[i]).outerWidth();
						 break;
					   }
					}
				if( amount < 0 ) {
                ...
                if( dir === 'next' && totalSpace - ( Math.abs( translation )  ) < visibleSpace ) {

					//amount = totalSpace - ( Math.abs( translation ) + visibleSpace );
                    //customized by viamagus move only 1/4 of visiblespace 
                    amount = totalSpace - ( Math.abs( translation ) + visibleSpace/4 );
